// query is a lightweight replacement of github.com/google/go-querystring that
// support time layout and location but with removed support for custom decoders
// and delimiter separated values.
package query
...
func reflectValue(values url.Values, val reflect.Value, scope string) error {
	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if sv.Kind() == reflect.Slice || sv.Kind() == reflect.Array {
			// NOTE: removed support for delimiter separated value (see https://github.com/google/go-querystring/blob/v1.1.0/query/encode.go#L211-L247)
			for i := 0; i < sv.Len(); i++ {
				k := name
...
func valueString(v reflect.Value, opts tagOptions, sf reflect.StructField) (string, error) {
	...
	if v.Type() == timeType {
		...
		fmtTag := sf.Tag.Get(xtime.StructTagFormat)
		if timeFmt := xtime.GetFormat(fmtTag); timeFmt != "" {
			timeFormat = timeFmt
		}

		var timeLocation *time.Location
		locTag := sf.Tag.Get(xtime.StructTagLocation)
		if timeLoc, ok := xtime.GetLocation(locTag); ok {
			timeLocation = timeLoc
		} else if locTag != "" {
			return "", errors.Errorf("invalid or unloaded location specified: %s", locTag)
		}
